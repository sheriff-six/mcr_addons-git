...
util.AddNetworkString( "NPianoNetwork" )
...
local function HookChair( ply, ent )
	...
	if IsValid( inst ) && inst.Base == "npiano_base" then
...
net.Receive( "NPianoNetwork", function( length, client )
	...
	if enum == INSTNET_PLAY then
		...
		if ent.Base != "npiano_base" then return end
...
concommand.Add( "instrument_leave", function( ply, cmd, args )
	...
	// Filter out non-instruments
	if !IsValid( ent ) || ent.Base != "npiano_base" then return end
