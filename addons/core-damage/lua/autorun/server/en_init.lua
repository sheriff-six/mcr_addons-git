...
function debug_print(msg)
  if (GetConVar("enhanceddamage_debug"):GetBool()) then
    print("DEBUG: " .. msg)
  end
end
...
function EnhancedDamage.HurtSound(ply,zone,level)
  ...
  debug_print("Voicetype: " .. voicetype)
  debug_print("Zone: " .. zone)
  ...
  if (chnc > chance) then
    if (ply:IsPlayer()) then
      ...
      if IsValid(weapon) then
        ...
        ply:SetNPCState(2)
        ply:AddEntityRelationship(attacker, 2, 99) -- Attempt to make it run away
      end
    end
    end
  end
