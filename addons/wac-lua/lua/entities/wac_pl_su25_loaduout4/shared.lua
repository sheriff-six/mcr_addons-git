...
ENT.Seats = {
	{
		...
		weapons = {"ГШ-30-2","X-29","ПТУР Вихрь","КАБ-500","РБК-500","Р-60",}
	},
}
...
ENT.Weapons = {
	...
	["КАБ-500"] = {
		...
		info = {
			...
			FireRate = 20,
			Kind = "bomb_kab500_wac"
		},
	},
	["X-29"] = {
		class = "sw_pod_kh29_wac",
		info = {
			Sequential = true,
			Pods = {
				Vector(-41, -108.4, 53.4),
				Vector(-31, -77.5, 54),
			},
			FireRate = 50,
			Ammo = 2,
...
ENT.WeaponAttachments = {
	vyihr_launcher1 = {
		...
		pos = Vector(-11, -108.4, 46.5),
		restrictPitch = true,
		restrictYaw = true,
	},
	kh29_1 = {
		model = "models/sw/avia/bombs/kh29.mdl",
		pos = Vector(-10, -77.5, 59),
		restrictPitch = true,
		restrictYaw = true,
	},
	vyihr_launcher2 = {
...
function ENT:DrawPilotHud()
	...
	if self:GetNWInt("seat_1_actwep") == 2 and self.weapons["X-29"] then
		surface.SetTextPos(150,450)
		local n = self.weapons["X-29"]:GetAmmo()
		surface.DrawText("Оружие:X-29" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 3 and self.weapons["ПТУР Вихрь"] then
		surface.SetTextPos(150,450)
		local n = self.weapons["ПТУР Вихрь"]:GetAmmo()
		surface.DrawText("Оружие:ПТУР Вихрь" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 4 and self.weapons["КАБ-500"] then
		surface.SetTextPos(170,450)
		local n = self.weapons["КАБ-500"]:GetAmmo()
		surface.DrawText("Оружие:КАБ-500" .. getspaces(n))
	end

	
	if self:GetNWInt("seat_1_actwep") == 5 and self.weapons["РБК-500"] then
		surface.SetTextPos(170,450)
		local n = self.weapons["РБК-500"]:GetAmmo()
		surface.DrawText("Оружие:РБК-500" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 6 and self.weapons["Р-60"] then
		surface.SetTextPos(170,450)
