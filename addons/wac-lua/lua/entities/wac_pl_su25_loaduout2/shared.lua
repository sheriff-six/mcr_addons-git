...
ENT.Seats = {
	{
		...
		weapons = {"ГШ-30-2","СППУ-22-01 №1","СППУ-22-01 №2","ФАБ-500М-62","Р-60",}
	},
}
...
ENT.WeaponAttachments = {
	sppu22_1 = {
		model = "models/sw/avia/bombs/sppu22.mdl",
		pos = Vector(-3, -108.4, 53.4),
		restrictPitch = true,
		restrictYaw = true,
	},
	sppu22_2 = {
		model = "models/sw/avia/bombs/sppu22.mdl",
		pos = Vector(6, -77.5, 54),
		restrictPitch = true,
		restrictYaw = true,
	},
	sppu22_3 = {
...
function ENT:DrawPilotHud()
	...
	if self:GetNWInt("seat_1_actwep") == 2 and self.weapons["СППУ-22-01 №1"] then
		surface.SetTextPos(130,450)
		local n = self.weapons["СППУ-22-01 №1"]:GetAmmo()
		surface.DrawText("Оружие:СППУ-22-01 №1" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 3 and self.weapons["СППУ-22-01 №2"] then
		surface.SetTextPos(130,450)
		local n = self.weapons["СППУ-22-01 №2"]:GetAmmo()
		surface.DrawText("Оружие:СППУ-22-01 №2" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 4 and self.weapons["ФАБ-500М-62"] then
		surface.SetTextPos(150,450)
		local n = self.weapons["ФАБ-500М-62"]:GetAmmo()
		surface.DrawText("Оружие:ФАБ-500М-62" .. getspaces(n))
	end

	if self:GetNWInt("seat_1_actwep") == 5 and self.weapons["Р-60"] then
		surface.SetTextPos(170,450)
