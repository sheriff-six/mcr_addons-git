...
ENT.Seats = {
	{
		...
		weapons = {"ГШ-30-2","С-8","ФАБ-250М-62","Р-60",}
	},
}
...
ENT.WeaponAttachments = {
	B8M1_1 = {
		model = "models/sw/avia/bombs/B8M1.mdl",
		pos = Vector(-41, -108.4, 53.4),
		restrictPitch = true,
		restrictYaw = true,
	},
	B8M1_2 = {
		model = "models/sw/avia/bombs/B8M1.mdl",
		pos = Vector(-31, -77.5, 54),
		restrictPitch = true,
		restrictYaw = true,
	},
	B8M1_3 = {
...
function ENT:DrawPilotHud()
	...
	if self:GetNWInt("health")>75 then
		surface.SetTextPos(300, 500)
		local health = self:GetNWInt("health")
		surface.DrawText(getspaces(health) .."%")
	end
	if self:GetNWInt("health")<75 then
		surface.SetTextPos(300, 500)
		local health = self:GetNWInt("health")
	...
	if self:GetNWInt("health")<50 then
		surface.SetTextPos(300, 500)
		local health = self:GetNWInt("health")
		surface.DrawText(getspaces(health) .."%")
	end
	if self:GetNWInt("health")<25 then
		surface.SetTextPos(300, 500)
		local health = self:GetNWInt("health")
		surface.DrawText(getspaces(health) .."%")
	end
	if self:GetNWInt("health")<5 then
		surface.SetTextPos(300, 500)
		local health = self:GetNWInt("health")
		surface.DrawText(getspaces(health) .."%")
	end
