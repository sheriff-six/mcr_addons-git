...
function SWEP:Initialize()
    -- Thank you DarkRP, your code is terrible
...
if CLIENT then
    ...
    function SWEP:CuffsHUD()
        ...
        if self:GetIsGagged() then
            TextPos = TextPos + 20
            draw.SimpleText("Тебе вставили кляп", "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
            draw.SimpleText("Тебе вставили кляп", "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
        end
        ...
        --[[if cvars.Bool("cuffs_allowbreakout") then
            ...
            if self.Owner:GetActiveWeapon() == self then
                str = string.format("%s чтобы развязаться", (input.LookupBinding("+attack") or "[Primary Fire]"):upper())
            else
...
local renderpos = {
    left = {
        ...
        pos = Vector(0.4, -0.15, -0.45),
        ang = Angle(90, 0, 0),
        scale = Vector(0.035, 0.035, 0.015)
    },
    right = {
        bone = "ValveBiped.Bip01_R_Wrist",
        pos = Vector(0.2, -0.15, 0.35),
        ang = Angle(100, 0, 0),
        scale = Vector(0.035, 0.035, 0.015)
    },
    rope = {
        l = Vector(-0.2, 1.3, -0.25),
        r = Vector(0.4, 1.4, -0.2)
    },
}
...
local wrender = {
    left = {
        ...
        pos = Vector(0, 0, 0),
        ang = Angle(90, 0, 0),
        scale = Vector(0.035, 0.035, 0.035)
    },
    right = {
