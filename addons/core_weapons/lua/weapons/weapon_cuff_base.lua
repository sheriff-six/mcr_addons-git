...
function SWEP:DrawHUD()
    ...
    local w, h = ScrW() / 2, ScrH() / 2
    draw.SimpleText("Связывание...", "HandcuffsText", w + 1, h + 31, Col.TextShadow, TEXT_ALIGN_CENTER)
    draw.SimpleText("Связывание...", "HandcuffsText", w, h + 30, Col.Text, TEXT_ALIGN_CENTER)
    surface.SetDrawColor(Col.BoxOutline)
    surface.DrawOutlinedRect(w - 101, h + 54, 202, 22)
    surface.SetDrawColor(Col.BoxBackground)
    surface.DrawRect(w - 100, h + 55, 200, 20)
...
local renderpos = {
    ...
    right = {
        ...
        pos = Vector(3.2, 2.1, 0.4),
        ang = Angle(-2, 0, 80),
        scale = Vector(0.045, 0.045, 0.03)
    },
    rope = {
        l = Vector(0, 0, 2.0),
        r = Vector(2.3, -1.9, 2.7)
    },
}
...
function SWEP:ViewModelDrawn(vm)
    ...
    if not self.RopeMat then
        self.RopeMat = Material(self.CuffRope)
    end
...
SWEP.wrender = {
    ...
    right = {
        bone = "ValveBiped.Bip01_R_Hand",
        pos = Vector(2.95, 2.5, -0.2),
        ang = Angle(30, 0, 90),
        scale = Vector(0.044, 0.044, 0.044)
    },
    rope = {
        l = Vector(0, 0, 2),
        r = Vector(3, -1.65, 1)
