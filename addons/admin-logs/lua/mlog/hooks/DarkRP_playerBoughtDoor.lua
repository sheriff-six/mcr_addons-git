...
hook.Add("playerBoughtPistol", HOOK.hook_name .. "door", function(ply,ent, price)
	if not IsValid(ply) then return end
	if not IsValid(ent) then return end

 	local plyName = mLog.getInfoLine(ply)
	mLog.Log("playerBoughtCustomEntity", plyName .. " bought a door for " .. (DarkRP and DarkRP.formatMoney and DarkRP.formatMoney(price) or price))
end)