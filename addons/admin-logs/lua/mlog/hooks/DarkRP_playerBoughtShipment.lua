...
hook.Add("playerBoughtShipment", HOOK.hook_name .. "shipment", function(ply, entTab, ent, price)
	if not IsValid(ply) then return end
	if not entTab then return end

 	local plyName = mLog.getInfoLine(ply)
	mLog.Log("playerBoughtCustomEntity", plyName .. " bought a shipment of " .. entTab.amount .. " " .. entTab.name .. "(s) for " .. (DarkRP and DarkRP.formatMoney and DarkRP.formatMoney(price) or price))
end)