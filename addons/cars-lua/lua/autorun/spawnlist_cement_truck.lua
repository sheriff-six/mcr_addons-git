local V = {
	Name = "Cement Truck",
	Model = "models/left4dead/vehicles/cement_truck.mdl",
	...
	Members = {
		...
		OnSpawn = function(ent)
			ent.mixerState = 0
			ent.mixerMoving = 0
			ent.mixerCooldown = false
			
			ent.mixerStart = CreateSound(ent, "left4dead/vehicles/mixer_start.wav" )
			ent.mixerStop = CreateSound(ent, "left4dead/vehicles/mixer_stop.wav" )
		end,
		
		OnTick = function(ent)
			local RPM = ent:GetRPM() / 1500
			...
			local function mixer()
				...
				if ply:KeyDown( 2048 ) and IsValid(ent:GetDriver()) then
					if ent.mixerMoving == 0 and !ent.mixerCooldown and ent:EngineActive() then
						ent.mixerMoving = 1
						
						ent.mixerStart:Play()
						ent.mixerStop:Stop()
						ent.mixerCooldown = true
			...
			if ent.mixerMoving == 1 then
				ent:SetPoseParameter("mixer_rotate", ( ent.mixerState ))
				ent.mixerState = ent.mixerState + RPM
			
				if ent.mixerState > 360 then
					ent.mixerState = 0
				end
			end
			
			if !ent:EngineActive() and ent.mixerMoving then
				ent.mixerMoving = 0
		...
		OnDelete = function(ent)
			if ent.mixerStart then
				ent.mixerStart:Stop()
			end
			if ent.mixerStop then
				ent.mixerStop:Stop()
			end
		end,
		
		CustomWheelPosFL = Vector(156.5,39,27),
		CustomWheelPosFR = Vector(156.5,-39,27),	
...
list.Set( "simfphys_vehicles", "sim_fphys_l4d_cement_truck", V )
...
list.Set( "simfphys_lights", "cement_truck", light_table)