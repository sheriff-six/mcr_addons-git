...
SCarAnim.handLeftBrakeAng = {}
SCarAnim.handLeftBrakeAng[1] = Angle(-30, 40, 0)
SCarAnim.handLeftBrakeAng[2] = Angle( 0, -30, 0)
SCarAnim.handLeftBrakeAng[3] = Angle( 0, 0, -30)

SCarAnim.handRightBrakeAng = {}
SCarAnim.handRightBrakeAng[1] = Angle(-30, 50, 0)
SCarAnim.handRightBrakeAng[2] = Angle( 30, -50, 0)
SCarAnim.handRightBrakeAng[3] = Angle( 0, 0, -30)


SCarAnim.hornState = 0
SCarAnim.prehorn = false
SCarAnim.hornDel = CurTime()
SCarAnim.hornTime = 1 / 0.1
SCarAnim.hornAng = {}
SCarAnim.hornAng[1] = Angle( -21.18, 7.3, -16.22 )
SCarAnim.hornAng[2] = Angle( 0, 14.42, -22.13)
...
SCarAnim.gearLeftAngles = {}
SCarAnim.gearLeftAngles[1] = Angle( -35, 51.63, -16.22 )
SCarAnim.gearLeftAngles[2] = Angle( 10.32, -39.83, -45.73 )
SCarAnim.gearLeftAngles[3] = Angle( 0, 16.22, 13.27 )

SCarAnim.gearRightAngles = {}
SCarAnim.gearRightAngles[1] = Angle( -15, 61.63, -16.22 )
SCarAnim.gearRightAngles[2] = Angle( 30.32, -69.83, -45.73 )
SCarAnim.gearRightAngles[3] = Angle( 0, 16.22, -90.27 )

SCarAnim.ignitionState = 0
SCarAnim.ignitionDel = CurTime()
SCarAnim.ignitionTime = 1 / 0.1
SCarAnim.ignitionAngles = {}
SCarAnim.ignitionAngles[1] = Angle( 0, 10, 0 )
SCarAnim.ignitionAngles[2] = Angle( 0, 0, 0 )
SCarAnim.ignitionAngles[3] = Angle( 0, 0, 10 )

SCarAnim.phoneState = 0
SCarAnim.phoneDel = CurTime()
SCarAnim.phoneTime = 1 / 0.1
SCarAnim.phoneAngles = {}
SCarAnim.phoneAngles[1] = Angle( 0, 0, 0 )
SCarAnim.phoneAngles[2] = Angle( 0, 0, 0 )
SCarAnim.phoneAngles[3] = Angle( 30, -50, 40 )
...
SCarAnim.brakeAng = {}
SCarAnim.brakeAng[1] = Angle( -5, 10, 0 )
SCarAnim.brakeAng[2] = Angle( -0, -3.18, -10)
SCarAnim.brakeAng[3] = Angle( 0, 20.93, 0 )
...
SCarAnim.throttAng[3] = Angle(1.47,12.13, 0)

SCarAnim.throttPlusAng = {}
SCarAnim.throttPlusAng[1] = Angle( 0, 10.32, 0)
...
hook.Add('CanExitVehicle', "MGD.ResetAnimsCar", function(veh, ply)
	ply.SCarAnim.doAnimationSwitch = false
	ply.SCarAnim.ResetSCarAnimations(ply)
	InitializePlayerCarAnimations(ply)
end )

function InitializePlayerCarAnimations(pl)
	pl.SCarAnim = {}
	pl.SCarHudHandler = {}

	for k, v in pairs(SCarAnim) do
		pl.SCarAnim[k] = v
	end

	for k, v in pairs(SCarHudHandler) do
		pl.SCarHudHandler[k] = v
	end
end

--hook.Add('PlayerInitialSpawn', 'MGD.InitializePlayerarAnimations', InitializePlayerCarAnimations)

--ManipulateBoneAngles is flawed. Makes the player disappear

--TRY PrePlayerDraw as hook.
function SCarAnim.ManipulateBoneAng(bone, ang, pl)  
	local boneInd = pl:LookupBone(bone)
	
	if bone and boneInd and boneInd >= 0 then
		pl:ManipulateBoneAngles( boneInd, ang)
	end
end
function SCarAnim.DoSCarAnimations(car, pl)
	pl.SCarAnim = pl.SCarAnim or {}
	local Steer = car:GetVehicleSteer()

	if Steer > 0.5 or Steer < -0.5 then
		if pl.SCarAnim.clutchState == 0 and pl.SCarAnim.hornState != 0 and pl.SCarAnim.handBrakeState != 0 then
			--SCarAnim.ResetSCarAnimations(pl)
			--return 
		end
	end
	

	pl.SCarAnim.tmpAng.p = 0 
	pl.SCarAnim.tmpAng.y = 0
	pl.SCarAnim.tmpAng.r = 0
	--------------------------DETECT GEAR CHANGE
	pl.SCarAnim.gearChange = false
	if pl.SCarAnim.lastGear != car:GetGear() then
		pl.SCarAnim.gearChange = true
		pl.SCarAnim.lastGear = car:GetGear()
	end

	--------------------------HORN
	if pl.SCarAnim.hornState == 0 and car:GetNWBool('HornKeyIsDown') then
		if Steer < 0.5 and Steer > -0.5 then
			pl.SCarAnim.hornState = 1
			pl.SCarAnim.hornDel = CurTime()
		else
			pl.SCarAnim.hornState = 0
		end
	end
 
	if pl.SCarAnim.hornState != 0 then
	
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.hornDel) * pl.SCarAnim.hornTime
		
		if pl.SCarAnim.hornState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )
		
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_UpperArm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.hornAng[1]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Forearm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.hornAng[2]), pl)
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Hand", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.hornAng[3]), pl)
		
		if pl.SCarAnim.animPer >= 1 and !car:GetNWBool('HornKeyIsDown') then
			pl.SCarAnim.hornState = 2 
			pl.SCarAnim.hornDel = CurTime()
		elseif pl.SCarAnim.hornState == 2 and pl.SCarAnim.animPer <= 0 then
			pl.SCarAnim.hornState = 0
		end
	end

	--------------------------HANDBRAKE			
	if pl.SCarAnim.prehandbrake != car:GetIsCruiseModeOn() then --and SCarHudHandler.CurGear == -3 then
		if !car.LimitSoundM then
		--if Steer < 0.5 and Steer > -0.5 then
			pl.SCarAnim.handBrakeState = 1
			pl.SCarAnim.handBrakeDel = CurTime()
			pl.SCarAnim.prehandbrake = car:GetIsCruiseModeOn()
		--else
		--	pl.SCarAnim.handBrakeState = 0
		--end
		else
			pl.SCarAnim.handBrakeState = 0
		end
	...
	if !car:GetHandBrakeEnabled() and car:GetIsCruiseModeOn() then
		if Steer < 0.5 and Steer > -0.5 then
			pl.SCarAnim.handBrakeState = 1
			pl.SCarAnim.handBrakeDel = CurTime()
		else
	...
	if pl.SCarAnim.handBrakeState == 1 or pl.SCarAnim.handBrakeState == 2 then
	
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.handBrakeDel) * pl.SCarAnim.handBrakeTime
		
		if pl.SCarAnim.handBrakeState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )
		if pl.SCarAnim.hornState == 0 then
			local ang_1, ang_2, ang_3 = pl.SCarAnim.gearAngles[1], pl.SCarAnim.gearAngles[2], pl.SCarAnim.gearAngles[3]

			if Steer > 0.5 then
				ang_1 = pl.SCarAnim.handRightBrakeAng[1]
				ang_2 = pl.SCarAnim.handRightBrakeAng[2]
				ang_3 = pl.SCarAnim.handRightBrakeAng[3]
			elseif Steer < -0.5 then
				ang_1 = pl.SCarAnim.handLeftBrakeAng[1]
				ang_2 = pl.SCarAnim.handLeftBrakeAng[2]
				ang_3 = pl.SCarAnim.handLeftBrakeAng[3]
			end

			SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_UpperArm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, ang_1), pl)
			SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Forearm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, ang_2), pl)
			SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Hand", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, ang_3), pl)		
		end

		if pl.SCarHudHandler.CurGear != -3 and pl.SCarAnim.animPer >= 1 and (!car:GetHandBrakeEnabled() or !car:GetIsCruiseModeOn()) then
			pl.SCarAnim.handBrakeState = 2 
			pl.SCarAnim.handBrakeDel = CurTime()
			
		elseif pl.SCarAnim.handBrakeState == 2 and pl.SCarAnim.animPer <= 0 then
			pl.SCarAnim.handBrakeState = 0
		end
	end				
	--------------------------GEAR
	if pl.SCarAnim.gearChange == true and pl.SCarAnim.gearState == 0 then
		if !car.LimitSoundM then
			pl.SCarAnim.gearState = 1
			pl.SCarAnim.gearDel = CurTime()
			pl.SCarAnim.handBrakeState = 0
		else
			pl.SCarAnim.gearState = 0
		end
	end
	
	if pl.SCarAnim.gearState == 1 or pl.SCarAnim.gearState == 2 then
	
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.gearDel) * pl.SCarAnim.gearTime
		
		if pl.SCarAnim.gearState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		...
		if Steer > 0.5 then
			ang_1 = pl.SCarAnim.gearRightAngles[1]
			ang_2 = pl.SCarAnim.gearRightAngles[2]
			ang_3 = pl.SCarAnim.gearRightAngles[3]
		elseif Steer < -0.5 then
			ang_1 = pl.SCarAnim.gearLeftAngles[1]
			ang_2 = pl.SCarAnim.gearLeftAngles[2]
			ang_3 = pl.SCarAnim.gearLeftAngles[3]
		end
		...
		if pl.SCarAnim.animPer >= 1 and pl.SCarAnim.gearState == 1 then
			pl.SCarAnim.gearState = 2
			pl.SCarAnim.gearDel = CurTime()	
		elseif pl.SCarAnim.animPer <= 0 and pl.SCarAnim.gearState == 2 then
			pl.SCarAnim.gearState = 0
		end
	end	
	
	---------------IGNITION
	if !car.StopEngineTime then car.StopEngineTime = CurTime() - 1 end

	if pl.SCarAnim.ignitionState == 0 and (car.LimitSoundM or (car.StopEngineTime and car.StopEngineTime > CurTime())) then
		--if Steer < 0.5 and Steer > -0.5 then
			pl.SCarAnim.ignitionState = 1
			pl.SCarAnim.hornDel = CurTime()
		--else
		--	pl.SCarAnim.ignitionState = 0
		--end
	end
	
	if pl.SCarAnim.ignitionState != 0 then
	
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.ignitionDel) * pl.SCarAnim.ignitionTime
		
		if pl.SCarAnim.ignitionState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )
		
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_UpperArm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.ignitionAngles[1]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Forearm", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.ignitionAngles[2]), pl)
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Hand", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.ignitionAngles[3]), pl)
		
		if pl.SCarAnim.animPer >= 1 and (!car.LimitSoundM and (car.StopEngineTime and car.StopEngineTime < CurTime())) then
			pl.SCarAnim.ignitionState = 2 
			pl.SCarAnim.ignitionDel = CurTime()
		elseif pl.SCarAnim.ignitionState == 2 and pl.SCarAnim.animPer <= 0 then
			pl.SCarAnim.ignitionState = 0
		end
	end
	
	--------------------------BRAKE PEDAL
	if pl.SCarAnim.brakeState == 0 and car.PressedKeys['S'] then
		pl.SCarAnim.brakeState = 1
		pl.SCarAnim.brakeDel = CurTime()
	elseif pl.SCarAnim.brakeState == 1 and !car.PressedKeys['S'] then
		pl.SCarAnim.brakeState = 2
		pl.SCarAnim.brakeDel = CurTime()			
	end

	if (pl.SCarAnim.brakeState == 1 or pl.SCarAnim.brakeState == 2) then
		
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.brakeDel) * pl.SCarAnim.brakeTime
		
		if pl.SCarAnim.brakeState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )				
		
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Thigh", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.brakeAng[1]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Calf", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.brakeAng[2]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Foot", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.brakeAng[3]), pl)			
		
		if pl.SCarAnim.brakeState == 2 and pl.SCarAnim.animPer <= 0 and !car.PressedKeys['S'] then
			pl.SCarAnim.brakeState = 0
		end
	end				

	--------------------------CLUTCH
	if pl.SCarAnim.clutchState == 0 and car:GetClutch() == 1 and car:GetGear() != 2 then
		pl.SCarAnim.clutchState = 1
		pl.SCarAnim.clutchDel = CurTime()
	end

	if pl.SCarAnim.clutchState == 1 or pl.SCarAnim.clutchState == 2 then
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.clutchDel) * pl.SCarAnim.clutchTime
		
		if pl.SCarAnim.clutchState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )
		
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_L_Thigh", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.clutchAng[1]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_L_Calf", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.clutchAng[2]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_L_Foot", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.clutchAng[3]), pl)

		if pl.SCarAnim.clutchState == 1 and pl.SCarAnim.gearChange == false and (pl.SCarAnim.brakeState == 0 or pl.SCarAnim.brakeState == 2) and pl.SCarAnim.animPer >= 1 and car:GetClutch() != 1 then
			pl.SCarAnim.clutchState = 2
			pl.SCarAnim.clutchDel = CurTime()
		elseif pl.SCarAnim.clutchState == 2 and pl.SCarAnim.animPer <= 0 then
			pl.SCarAnim.clutchState = 0
		end
	end
	
	
	--------------------------GAS PEDAL
	if pl.SCarAnim.gearState != 1 and pl.SCarAnim.throttleState == 0 and car:GetThrottle() > 0 then
		pl.SCarAnim.throttleState = 1
		pl.SCarAnim.throttleDel = CurTime()
	elseif pl.SCarAnim.throttleState == 1 and car:GetThrottle() == 0 then
		pl.SCarAnim.throttleState = 2
		pl.SCarAnim.throttleDel = CurTime()	
	end

	if pl.SCarAnim.throttleState == 1 or pl.SCarAnim.throttleState == 2 then
		
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.throttleDel) * pl.SCarAnim.throttleTime
		
		if pl.SCarAnim.throttleState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		...
		if car:GetThrottle() != 1 then
			ang = Angle(0, 0, 0)
		end

		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Thigh", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.throttAng[1]), pl)	
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Foot", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.throttAng[2]), pl)
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Foot", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.throttAng[3] + ang), pl)	

		if pl.SCarAnim.throttleState == 2 and pl.SCarAnim.animPer == 0 then
			pl.SCarAnim.throttleState = 0
		end
	end	

	--------------------------PLUS THROTTLE
	--[[if pl.SCarAnim.gearState != 1 and pl.SCarAnim.throttlePlusState == 0 and car:GetThrottle() == 1 then
		pl.SCarAnim.throttlePlusState = 1
		pl.SCarAnim.throttleDel = CurTime()
	elseif pl.SCarAnim.throttlePlusState == 1 and car:GetThrottle() != 1 then
		pl.SCarAnim.throttlePlusState = 2
		pl.SCarAnim.throttleDel = CurTime()	
	end

	if pl.SCarAnim.throttlePlusState == 1 or pl.SCarAnim.throttlePlusState == 2 then
		
		pl.SCarAnim.animPer = (CurTime() - pl.SCarAnim.throttleDel) * pl.SCarAnim.throttleTime
		
		if pl.SCarAnim.throttlePlusState == 2 then
			pl.SCarAnim.animPer = 1 - pl.SCarAnim.animPer
		end
		
		pl.SCarAnim.animPer = math.Clamp( pl.SCarAnim.animPer, 0, 1 )
		
		SCarAnim.ManipulateBoneAng("ValveBiped.Bip01_R_Foot", LerpAngle( pl.SCarAnim.animPer, pl.SCarAnim.tmpAng, pl.SCarAnim.throttPlusAng[1]), pl)	

		if pl.SCarAnim.throttlePlusState == 2 and pl.SCarAnim.animPer == 0 then
			pl.SCarAnim.throttlePlusState = 0
		end
	end]]
end

function SCarAnim.ResetSCarAnimations(pl)

	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_UpperArm")
	print('pl: ', pl, 'boneInd:',  pl.SCarAnim.boneInd)
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))
	
	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_Forearm")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))			
	
	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_Hand")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))
	
	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_Thigh")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))
	
	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_Calf")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))

	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_R_Foot")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))

	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_L_Thigh")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))
	
	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_L_Calf")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))

	pl.SCarAnim.boneInd = pl:LookupBone("ValveBiped.Bip01_L_Foot")
	pl:ManipulateBoneAngles( pl.SCarAnim.boneInd, Angle(0,0,0))

end 

function SCarAnim.SCar_Animation_Override(ply, vel, maxSeqGround)
    if !ply.SCarAnim or (ply.SCarAnim and !ply.SCarAnim.DoSCarAnimations) then
        InitializePlayerCarAnimations(ply)
        return
    end
    
	ply.SCarAnim = ply.SCarAnim or {}
	ply.SCarAnim.isSeat = 0
	local car

	if IsValid(ply:GetVehicle()) then
		car = ply:GetVehicle():GetParent()
	end
	
	if !simfphys.IsCar(car) then car = nil end
	if IsValid(car) and car.DriverSeat != ply:GetVehicle() then car = nil end

	if car then
		ply.SCarAnim.isSeat = 1
	else
		ply.SCarAnim.isSeat = 0
	end		
	if ply.SCarAnim.isSeat >= 1 and ply.SCarAnim.doAnimationSwitch == false then
		ply.SCarAnim.doAnimationSwitch = true
		ply.SCarAnim.startAnimatingDelay = CurTime() + 0.2
	elseif ply.SCarAnim.isSeat <= 0 and ply.SCarAnim.doAnimationSwitch == true then
		ply.SCarAnim.doAnimationSwitch = false
		ply.SCarAnim.ResetSCarAnimations(ply)
	end
    
    if !ply.SCarAnim.DoSCarAnimations then
       InitializeCarAnimations(ply)
    end

	if ply.SCarAnim.doAnimationSwitch and SCarAnim.startAnimatingDelay < CurTime() and car and ply.SCarAnim.DoSCarAnimations then
		ply.SCarAnim.DoSCarAnimations(car, ply)
	end 
end
hook.Add("UpdateAnimation", "SCarAnim.SCar_Animation_Override", SCarAnim.SCar_Animation_Override) 

hook.Add("simfphysUse", "simfphysAnims", function(veh, ply)
	if ply.SCarAnim then
		ply.SCarAnim.ResetSCarAnimations(ply)
	end
end )
