...
if CLIENT then
	...
	language.Add( "Tool.precision.desc", "Более лёгкое и аккуратное строительство" )
	language.Add( "Tool.precision.0", "Primary: Передвинуть/Применить | Secondary: Толкать | Reload: Притягивать" )
	language.Add( "Tool.precision.1", "Нажмите ещё куда-нибудь для применение инструмента. (Смените оружие для отмены)" )
	language.Add( "Tool.precision.2", "Поворачивайте вправо и влево для кручения (Удерживайте ПКМ и R для смены осей)" )
	...
	local function AddDefControls( Panel )
		...
		Panel:AddControl( "Checkbox", { Label = "Толкание/Притягивание в процентакх", Command = "precision_nudgepercent", Description = "Unchecked = Exact units, Checked = takes % of width from target prop when pushing/pulling" } )
		...
		if ( mode == 1 ) then
			list:AddLine(" 1 ->Применить<- (Применить настройки)")
		else
			list:AddLine(" 1   Применить   (Применить настройки)")
		end
		if ( mode == 2 ) then
			list:AddLine(" 2 ->Поворот<- (Поворот объекта без перемещения)")
		else
			list:AddLine(" 2   Поворот   (Поворот объекта без перемещения)")
		end
		if ( mode == 3 ) then
			list:AddLine(" 3 ->Перемещение<- (Удобно в строительстве!)")
		else
			list:AddLine(" 3   Перемещение   (Удобно в строительстве!)")
		end
		if ( mode == 4 ) then
			list:AddLine(" 4 ->Сварка<-")
		else
			list:AddLine(" 4   Сварка")
		end
		if ( mode == 5 ) then
		...
		if ( mode == 6 ) then
			list:AddLine(" 6 ->Шарнир<-")
		else
			list:AddLine(" 6   Шарнир")
		end
		if ( user >= 2 ) then
			if ( mode == 7 ) then
				list:AddLine(" 7 ->Adv Шарнир<-")
			else
				list:AddLine(" 7   Adv Шарнир")
			end
			if ( mode == 8 ) then
				list:AddLine(" 8 ->Слайдер<-")
			else
				list:AddLine(" 8   Слайдер")
			end
		end
		if ( user >= 3 ) then
			if ( mode == 9 ) then
				list:AddLine(" 9 ->Парент<- (Как сварка, только без коллизии)")
			else
				list:AddLine(" 9   Парент   (Как сварка, только без коллизии)")
			end
			if ( mode == 10 ) then
				list:AddLine("10 ->Починка<- (Попытается починить летающие конструкции)")
			else
				list:AddLine("10   Починка   (Попытается починить летающие конструкции)")
			end
		end
		if ( mode == 11 ) then
			list:AddLine("11 ->Отмена<- (Убрать выбранные инструменты)")
		else
			list:AddLine("11   Отмена   (Убрать выбранные инструменты)")
		end
		list:SortByColumn(1)
		...
		if ( mode >= 4 && mode <= 8 ) then
			Panel:AddControl( "Checkbox", { Label = "Переместить цель? ('Простой' режим)", Command = "precision_move", Description = "Uncheck this to apply the constraint without altering positions." } )
		end
		if (  mode >= 3 && mode <= 8 ) then
			Panel:AddControl( "Checkbox", { Label = "Повернуть цель? (Поворот после перемещения)", Command = "precision_rotate", Description = "Uncheck this to remove the extra click for rotation. Handy for speed building." } )
			...
			Panel:AddControl( "Slider",  { Label	= "Дистанция Snap",
					Type	= "Float",
					Min		= 0,
					Max		= 10,
					Command = "precision_offset",
					Description = "Distance offset between joined props.  Type in negative to inset when moving."}	 )
			Panel:AddControl( "Checkbox", { Label = "Snap дистанция в процентах", Command = "precision_offsetpercent", Description = "Unchecked = Exact units, Checked = takes % of width from second prop" } )
		end
		if ( mode >= 2 && mode <= 8 ) then
			Panel:AddControl( "Slider",  { Label	= "Угол поворота",
					Type	= "Float",
					Min		= 0.02,
					Max		= 90,
		...
		if ( mode <= 8 ) then
			Panel:AddControl( "Checkbox", { Label = "Зафризить проп", Command = "precision_freeze", Description = "Freeze props when this tool is used" } )

			if ( mode >= 3 && mode <= 8 ) then
				Panel:AddControl( "Checkbox", { Label = "Снять коллизию", Command = "precision_nocollide", Description = "Nocollide pairs of props when this tool is used. Note: No current way to remove this constraint when used alone."  } )
			end
		end

		if ( user >= 2 || mode == 1 ) then
			if ( (mode >= 3 && mode <= 8) || mode == 1 ) then
				Panel:AddControl( "Checkbox", { Label = "Автовыравнивание по миру (по 45 градусов)", Command = "precision_autorotate", Description = "Rotates to the nearest world axis (similar to holding sprint and use with physgun)"  } )
			end

			if ( mode == 1 ) then
				Panel:AddControl( "Checkbox", { Label = "Тень цели", Command = "precision_ShadowDisable", Description = "Disables shadows cast from the prop"  } )
			end
		end

		if ( user >= 3 ) then
			if ( mode == 1 ) then //apply
				Panel:AddControl( "Checkbox", { Label = "Колизия только к игрокам", Command = "precision_nocollideall", Description = "Nocollides the first prop to everything and the world (except players collide with it). Warning: don't let it fall away through the world."  } )
				Panel:AddControl( "Checkbox", { Label = "Отключить физган у объекта", Command = "precision_physdisable", Description = "Disables physics on the first prop (gravity, being shot etc won't effect it)"  } )
				Panel:AddControl( "Checkbox", { Label = "Продв: Разрешить применять Физган к PhysDisable объектам", Command = "precision_allowphysgun", Description = "Disabled to stop accidents, use if you want to be able to manually move props after phyics disabling them (may break clipboxes)."  } )
				
				//Panel:AddControl( "Checkbox", { Label = "Drag", Command = "precision_drag", Description = ""  } )
			end
			if ( mode == 9 ) then //parent
				Panel:AddControl( "Checkbox", { Label = "Продв: Разрешить применять Физган к Паренченым объектам", Command = "precision_allowphysgun", Description = "Disabled to stop accidents, use this if you want to play with the parenting hierarchy etc."  } )
			end
		end
		if ( user >= 2 ) then
			if ( mode != 2 && mode != 3 && mode != 10 ) then //todo: entire contrap move/rotate support
				Panel:AddControl( "Checkbox", { Label = "Использовать всё что связано с целью.", Command = "precision_entirecontrap", Description = "For mass constraining or removal or nudging or applying of things. Yay generic."  } )
			end
		end

		if ( user >= 2 ) then
			if ( (mode >= 4 && mode <= 7) ) then //breakable constraint
				Panel:AddControl( "Slider",  { Label	= "Сила разрыва",
						Type	= "Float",
						Min		= 0.0,
						Max		= 5000,
			...
			if ( mode == 5 || mode == 6 || mode == 7 ) then //axis or ballsocket
				Panel:AddControl( "Slider",  { Label	= "Точка остановки",
						Type	= "Float",
						Min		= 0.0,
						Max		= 5000,
		...
		if ( mode == 5 ) then //axis
			Panel:AddControl( "Slider",  { Label	= "Трение оси",
					Type	= "Float",
					Min		= 0.0,
					Max		= 100,
		...
		if ( mode ==7 ) then //adv ballsocket
			Panel:AddControl( "Slider",  { Label	= "X Поворот (минимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "X Поворот (максимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "Y Поворот (минимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "Y Поворот (максимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "Z Поворот (минимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "Z Поворот (максимум)",
					Type	= "Float",
					Min		= -180,
					Max		= 180,
			...
			Panel:AddControl( "Slider",  { Label	= "X Поворот (трение)",
					Type	= "Float",
					Min		= 0,
					Max		= 100,
			...
			Panel:AddControl( "Slider",  { Label	= "Y Поворот (трение)",
					Type	= "Float",
					Min		= 0,
					Max		= 100,
			...
			Panel:AddControl( "Slider",  { Label	= "Z Поворот (трение)",
					Type	= "Float",
					Min		= 0,
					Max		= 100,
			...
			Panel:AddControl( "Checkbox", { Label = "Свободное Движение", Command = "precision_FreeMov", Description = "Only lock relative rotation, not position?"  } )
		end

		if ( mode == 8 ) then //slider
			Panel:AddControl( "Slider",  { Label	= "Длина Слайдера",
					Type	= "Float",
					Min		= 0.0,
					Max		= 10,
			...
			Panel:AddControl( "Checkbox", { Label = "Отключить стабилизацию слайдера", Command = "precision_disablesliderfix", Description = "Fix being separate X/Y/Z advanced ballsocket locks between the props.  This stops most spaz caused by rotation, but not spaz caused by displacement." } )
		...
		if ( showgenmenu == 1 ) then
			Panel:AddControl( "Button", { Label = "\\/ Доп. Опции \\/", Command = "precision_generalmenu", Description = "Collapse menu"  } )




		local params = {Label = "Уровень пользователя",Description = "Shows options appropriate to user experience level", MenuButton = "0", Height = 67, Options = {}}
		if ( user == 1 ) then
			params.Options[" 1 ->Нормальный<-"] = { precision_setuser = "1" }
		else
			params.Options[" 1   Нормальный"] = { precision_setuser = "1" }
		end
		if ( user == 2 ) then
		...
		if ( user == 3 ) then
			params.Options[" 3 ->Профессиональный<-"] = { precision_setuser = "3" }
		else
			params.Options[" 3   Профессиональный"] = { precision_setuser = "3" }
		end
		...
		Panel:AddControl( "ListBox", params )
			...
			Panel:AddControl( "Checkbox", { Label = "Включить отклик инструмента?", Command = "precision_enablefeedback", Description = "Toggle for feedback messages incase they get annoying"  } )
			Panel:AddControl( "Checkbox", { Label = "Вкл = Отклик в чат, Выкл = Сообщение по центру", Command = "precision_chatfeedback", Description = "Chat too cluttered? Can have messages centre screen instead"  } )
			...
			Panel:AddControl( "Checkbox", { Label = "Добавить толк./прит. в лист отмены", Command = "precision_nudgeundo", Description = "For if you're in danger of nudging somthing to where you can't reach it"  } )
			Panel:AddControl( "Checkbox", { Label = "Добавить перемещение в лист отмены", Command = "precision_moveundo", Description = "So you don't have to secondary fire with nocollide to undo mistakes"  } )
			Panel:AddControl( "Checkbox", { Label = "Добавить поворот в лист отмены", Command = "precision_rotateundo", Description = "So you can find the exact rotation value easier"  } )
			Panel:AddControl( "Button", { Label = "ВЕРНУТЬ К ЗАВОДСКИМ НАСТРОЙКАМ", Command = "precision_defaultrestore", Description = "Collapse menu"  } )
		else
			Panel:AddControl( "Button", { Label = "-- Доп. Функции --", Command = "precision_generalmenu", Description = "Expand menu"  } )
			if ( user == 1 ) then
				Panel:AddControl( "Label", { Text = "(Note: Если хотите помочь в переводе данного аддона, то пишите мне в ВК: vk.com/johnwebm.)", Description = "" }  )
			end
		end
	end
