...
do
  ...
  _class_0 = setmetatable({
    __init = function(self, copyfrom)
      ...
      if copyfrom then
        return DLib.LTakeDamageInfo.CopyInto(copyfrom, self)
      end
    end,
    __base = _base_0,
...
DLib.LTakeDamageInfo.CopyInto = function(objectSource, self)
  do
    self:SetAmmoType(objectSource:GetAmmoType())
    self:SetAttacker(objectSource:GetAttacker())
    if self.SetBaseDamage and objectSource.SetBaseDamage then
      self:SetBaseDamage(objectSource:GetBaseDamage())
    end
    self:SetDamage(objectSource:GetDamage())
    self:SetDamageBonus(objectSource:GetDamageBonus())
    self:SetDamageCustom(objectSource:GetDamageCustom())
    self:SetDamageForce(objectSource:GetDamageForce())
    self:SetDamagePosition(objectSource:GetDamagePosition())
    self:SetDamageType(objectSource:GetDamageType())
    self:SetInflictor(objectSource:GetInflictor())
    self:SetMaxDamage(objectSource:GetMaxDamage())
    self:SetReportedPosition(objectSource:GetReportedPosition())
  end
  return self
end
