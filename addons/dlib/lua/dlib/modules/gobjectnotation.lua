...
GON.FindProvider = function(identity)
  return GON.IdentityRegistry[identity] or false
end
...
GON.RegisterProvider = function(provider, should_put)
  local identity = provider:GetIdentity()
  local identify = provider:LuaTypeIdentify()
...
do
  ...
  local _base_0 = {
    ...
    FindInHeap = function(self, value)
      if isnumber(value) and value ~= value then
        return self.nan_value or false
      end
      if self._heap then
    ...
    GetIdentityID = function(self, identity)
      ...
      if _get >= 0x100 then
        error('Too many different types in a single file! 255 is the maximum!')
      end
    ...
    AddToHeap = function(self, value)
      ...
      if provider and not provider:Ask(value, ltype) then
        provider = nil
      end
      ...
      if ltype == 'number' and value ~= value then
        self.nan_value = serialized
      else
        if self._heap then
    ...
    AnalyzeSize = function(self)
      local size = 12 + 4 + 1
      if self.next_reg_id ~= 0 then
        size = size + 1
        for i = 0, self.next_reg_id - 1 do
          size = size + #self.identity_registry[i] + 1
        end
      else
        size = size + 2
      end
      ...
      for _index_0 = 1, #_list_0 do
        ...
        if provider:IsKnownValue() then
          size = size + 2
          local analyze = provider:AnalyzeSize()
          if not analyze then
            return false
          end
          size = size + analyze
        else
          size = size + (2 + #provider:BinaryData())
        end
      end
      if self.root then
        size = size + (self:IsHeapBig() and 4 or 2)
      end
      return size
    end,
    WriteHeader = function(self, bytesbuffer)
      ...
      if self.next_reg_id ~= 0 then
        ...
        return bytesbuffer:WriteUShort(0)
      end
    end,
    ...
    ReadHeader = function(self, bytesbuffer)
      ...
      if read == '\xF7\x7FDLib.GON\x00\x01' then
        ...
        self.long_heap = true
        return true
      elseif read == '\xF7\x7FDLib.GON\x00\x02' or read == '\xF7\x7FDLib.GON\x00\x03' then
        self.old_value_length = read == '\xF7\x7FDLib.GON\x00\x02'
        self.identity_registry = { }
        self._identity_registry = { }
        self.next_reg_id = bytesbuffer:ReadUByte() + 1
        for i = 0, self.next_reg_id - 1 do
          read = bytesbuffer:ReadString()
          if read == '' and self.next_reg_id == 0 then
            return 
          else
            self.identity_registry[i] = read
            self._identity_registry[read] = i
    ...
    ReadHeap = function(self, bytesbuffer)
      ...
      for i = 1, amount do
        ...
        local len = self.old_value_length and bytesbuffer:ReadUInt16() or bytesbuffer:ReadUInt32()
        if not provider then
          ...
          p._identity_id = iid
          local realvalue
          if p:IsInstantValue() then
            realvalue = p:GetValue()
          end
          if realvalue ~= nil then
            if isnumber(realvalue) and realvalue ~= realvalue then
              self.nan_value = p
            else
              if self._heap then
                self._heap[realvalue] = p
              end
            end
          end
          local pos2 = bytesbuffer:Tell()
    ...
    CreateBuffer = function(self)
      do
        local analyze = self:AnalyzeSize()
        if analyze then
          local bytesbuffer = DLib.BytesBuffer.Allocate(analyze)
...
do
  ...
  self.Ask = function(self, value, ltype)
    ...
    return ltype == 'number' and value == value
  end
  self.GetIdentity = function(self)
...
do
  ...
  local _parent_0 = GON.IDataProvider
  local _base_0 = {
    Serialize = function(self, bytesbuffer) end,
    AnalyzeSize = function(self)
      return 0
    end,
    GetValue = function(self)
  ...
  self.Deserialize = function(self, bytesbuffer, structure, heapid, length)
    return GON.NaNProvider(structure, heapid)
  end
  if _parent_0.__inherited then
    _parent_0.__inherited(_parent_0, _class_0)
  end
  GON.NaNProvider = _class_0
end
do
  local _class_0
  local _parent_0 = GON.IDataProvider
